...
public class ColossusEnchantment extends Enchantment
{
    ...
    protected static boolean filterTargets(PlayerEntity caster, Object target)
    {
        if(target instanceof BlockState)
            return ((BlockState) target).getMaterial().isSolid();
    ...
    static
    {
        PlayerItemUseCallback.EVENT.register((player, hand, stack) -> {
            ...
            ColossusEntity colossus = new ColossusEntity(ModEntities.COLOSSUS, player.getWorld());
    ...
    protected static Pair<Vec3d, Direction> getColossusSpawningPos(PlayerEntity user)
    {
        ...
        if(result.getType().equals(HitResult.Type.BLOCK))
        {
            ...
            Vec3d spawnPos = Vec3d.ofCenter(blockPos).add(new Vec3d(facing.getUnitVector()).multiply(0.6));
        ...
        if(result.getType().equals(HitResult.Type.ENTITY))
        {
            ...
            while(!user.getWorld().getBlockState(mutable.move(Direction.DOWN)).getMaterial().isSolid() && mutable.getY() >= 0);

            if(mutable.getY() < 0)
                return null;

            Vec3d spawnPos = Vec3d.ofCenter(mutable).add(0.5F, 1.1F, 0.5F);
